---
title: "Determine EEZ of vessels"
...
In this script, we prep EEZ data and match that to the vessel detection data. 

The vessel detection data only includes the lat/lon points, and does not specify which EEZ each point is in. We will use EEZ data from ([Marine Regions V11](https://www.vliz.be/en/imis?dasid=6316&doiid=386)), and match that EEZ data to the vessel detection data. For the EEZ regions, we will use flag countries represented in the data from Rousseau et al. 2019. We match to flag countries in this data because 1) the Rousseau data is the effort data we are allocating, and 2) we are assuming for these models that where a vessel is detected is the vessel's flag country. This assumption is made because we are modeling artisanal fisheries, and we assume artisanal fisheries can only fish within their EEZ (in line with SAUP, Rousseau, and Watson data, Skerritt et al. 2023):
...
Read in raw Rousseau country level data and filter for artisanal fisheries
...
Read in raw EEZ data from Marine Regions. You will need to download this to your large file storage first: https://www.vliz.be/en/imis?dasid=6316&doiid=386
...
data_grid %>%
  ...
  ungroup()  %>%
  as_tibble() %>%
  write_csv(here::here("data/model_features/global_grid.csv")) # we save this "global_grid" as our analysis grid
...
Take the EEZ data from above and extract to our grid, saving a column for the eez country. 

We will also exclude any uninhabited areas, as defined by the Ocean Health Index (OHI): https://github.com/OHI-Science/ohiprep_v2025/blob/2a0ba2f1f25cc4e2331ffcc67f3006d3988c50ba/globalprep/spatial/v2017/output/rgn_uninhabited_islands.csv

```{r}
rousseau_eezs <- unique(rousseau_eff$flag_fin) #164

eez_test <- eez_raw %>%
  st_drop_geometry() # just take a look at what columns there are
...
## need to filter out these places because they are uninhabited, and thus won't have artisanal fisheries
...
sort(setdiff(rousseau_eezs, eez_lookup_ids$eez_sovereign)) # [1] "BMU" "COK" "CUW" "FRO" "GLP" "GRL" "GUF" "GUM" "HKG" "MYT" "NCL" "NIU" "PRI" "PYF" "RAA" "RAM" "REU" "TWN" "UKR" "WLF"; # missing these because they are all some overlapping claims
...
# Filter for those places and add back into the EEZ data we will extract to our grid 
...
write.csv(eez_lookup_fix, here("data/model_features/eez/eez_lookup.csv"), row.names = FALSE) # now save a lookup

# filter for sovereign nations as above
...
# filter for territories or overlapping claims 
...
sort(setdiff(rousseau_eezs, eez_all$eez_sovereign))  # just hong kong, good, as this is not included in the EEZ data
...
```{r}
  ...
  qs::qsave(save_data_grid_eez, here("data/model_features/eez/int/eez_intersection.qs")) # save as we go just in case, since this takes so long to run...
  ...
  data_grid_eez_3_missing <- data_grid_eez_2_fix %>%
    left_join(data_grid %>% st_drop_geometry()) %>%
    filter(eez_sovereign %in% c("SGP", "SVN", "JOR")) %>% # need to fix these, since they were incorrectly assigned for some reason
  ...
  data_grid_eez_3_fin <- data_grid_eez_3 %>%
    dplyr::select(-fraction_eez_overlap) %>%
      ...
      filter(!(pixel_id == 96964 & eez_sovereign == "IDN")) %>% # re-assign these regions to be correct
  ...
  data_grid_high_seas <- data_grid_eez_3_fin %>%
    ...
    dplyr::distinct(pixel_id, eez_id) # now join to our grid and lookup table to get iso3c
 ...
 missing <- setdiff(unique(eez_lookup$eez_id), unique(data_grid_high_seas$eez_id)) # see what is missing
 ...
 test <- eez_lookup %>%
  ...
  write.csv(data_grid_high_seas, here("data/model_features/eez/eez.csv"), row.names = FALSE) # save

  check <-   read.csv(here("data/model_features/eez/eez.csv"))

```

See if we can add HKG to the data, using the EEZ data from Rousseau et al 2024 (it will only be 1 pixel anyways, I believe): https://github.com/Global-Fishing-Effort/RousseauEtAl2023/blob/main/Data/Cells_LatLon_EEZ.csv
...
eez_data <- read.csv(here("data/model_features/eez/eez.csv")) # read in our eez data created above

eez_lookup <- read.csv(here("data/model_features/eez/eez_lookup.csv"))

# read in eez data from Rousseau
...
eez_rousseau_hkg <- eez_rousseau_raw %>%
    filter(fa_oname %in% c("Hong Kong", "Mexico")) %>% # pick 2 countries because making a rast out of just 1 pixel (HKG) won't work
  dplyr::select(lon, lat, c_number) %>%
  rast(., type = "xyz") %>%
  terra::resample(., data_grid_rast, method = "bilinear") %>%
  as.data.frame(., xy = TRUE) %>%
  left_join(test) %>%
  filter(fa_oname == "Hong Kong") %>% # now filter for only HKG after extracting from our grid
...
write.csv(eez_lookup_fix, here("data/model_features/eez/eez_lookup_fix.csv"), row.names = FALSE) # re-save eez files
