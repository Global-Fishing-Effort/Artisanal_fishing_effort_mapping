---
title: "Aggregate vessel data and calculate proportions Sentinel"
output: html_document
date: "2025-07-22"
---

## Summary
Here we aggregate the Sentinel-2 data to 0.5 by 0.5 degree, matching our analysis grid
...
vessels_raw <- lapply(list.files(file.path(rdsi_dir, "prep/sentinel_2_prepped/prepped_10m_res"), full.names = TRUE), read.csv) %>%
  bind_rows()
...
Categorize vessel lengths in the raw detection data to match those contained in Rousseau et al. 
...
vessels_length_fix <- vessels_raw %>% 
  ...
  dplyr::group_by(lon, lat, length_m_inferred, year, length_category) %>%
  summarise(nv = n()) %>%
  ungroup()
...
Aggregate and save data to 0.5 degree
...
mapview::mapview(test, xcol = "x", ycol = "y") # yes. 
...
test <- vessel_agg_5_flag %>%
  filter(is.na(pixel_id)) # 0 


unique(vessel_agg_5_flag$flag_fin) # 171 flags
length(unique(vessel_agg_5_flag$eez_id)) # 171

# check against Rousseau

setdiff(unique(rousseau_eff$country), unique(vessel_agg_5_flag$flag_fin)) # 0
...
Now calculate the proportion of each flag country's vessels that are in each cell for each year and length category
...
qs::qsave(vessel_agg_5_props, here("data/vessels/vessels_prop_05_deg_sentinel.qs"))
