...
Here we save the data from the modelling in the previous steps in concise datasets, which we then upload to Zenodo. 
...
foreach(flag = all_flags, .packages = c("qs", "dplyr", "tidyverse", "glue", "countrycode")) %dopar% {
  ...
  write.csv(all_data_fin, file.path(rdsi_dir, glue("prep/random_forest/zenodo_data/artisanal_skylight_sentinel/mapped_by_flag_country_artisanal/model_preds_1950_2017_{flag}.csv")), row.names = FALSE)
}
...
## save as one large csv
  ...
  all_files_flag <- list.files(file.path(rdsi_dir, "prep/random_forest/zenodo_data/artisanal_skylight_sentinel/mapped_by_flag_country_artisanal/"), full.names = TRUE)
...
## Lets save the large map files as zip file as well, in the terminal:
# cd /home/ubuntu/data_storage/prep/random_forest/zenodo_data/artisanal_skylight_sentinel/
# zip -r mapped_by_flag_country_artisanal.zip mapped_by_flag_country_artisanal_artisanal
# zip mapped_artisanal_effort_predictions_1950_2017.zip mapped_artisanal_effort_predictions_1950_2017.csv
...
missing_data <- hist_fish_data %>%
  filter(is.na(proportion_nv_modeled)) # good, none missing

# write.csv(hist_fish_data, file.path(rdsi_dir, "prep/random_forest/zenodo_data/artisanal_skylight_sentinel/known_artisanal_effort_rousseau.csv"), row.names = FALSE) # no longer necesssary 
