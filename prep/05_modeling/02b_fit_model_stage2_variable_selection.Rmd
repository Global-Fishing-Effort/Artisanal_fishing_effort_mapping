...
model_data_flag <- model_data %>%
  dplyr::select(lon, lat, flag_fin, length_category, year, prop_vessels) %>%
...
model <- ranger(
  ...
  data = train,
  num.trees = 500, # change to 500 trees?
  importance = "impurity",  
  write.forest = TRUE,
...
if(!file.exists(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_{flag}_24.qs")))){
tic()
  model <- ranger(
  formula = full_model_formula,
  data = train,
...
toc() 
  ...
  model <- qs::qread(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_{flag}_24.qs")))
