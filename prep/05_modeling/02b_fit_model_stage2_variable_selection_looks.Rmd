---
title: "Fit prediction model for fishing vessels: stage 2"
...
We use the data compiled in the previous scripts to fit a random forest regression model to predict the intensity of fishing vessels occurring in cells. Our response variable is the logit of the smoothed per-look vessel detection rate (detections/looks). We used the logit-transformed smoothed vessel detection rate per look (detections/looks) to reduce sampling bias arising from unequal satellite coverage.
...
model_data <- qread(here("data/vessels/vessels_prop_05_deg_overpass_all.qs")) %>%
  mutate(year = as.numeric(year)) %>%
  ungroup() %>%
  filter(year < 2025) %>%
  mutate(log_looks = log(looks)) %>%
  filter(D > 0, looks > 0) %>%
  mutate(
    r_raw = D / looks,
    r_shr = (D + a) / (looks + a + b),
    r_clipped = pmin(pmax(r_shr, eps), 1 - eps),
    y_logit = qlogis(r_clipped)) # y_logit is our response variable
...
length(missing_flags) #  these are flags that are in the rousseau data but not in the skylight data, meaning we can't make predictions on them by flag models

flags <- setdiff(flags, missing_flags) # missing 0

stage_2_path <- file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning")
...
flags_to_run <- setdiff(flags, stage_2_flags)
...
model_data_flag <- model_data %>%
  dplyr::select(lon, lat, flag_fin, length_category, year, D, looks, log_looks, y_logit) %>%
  filter(flag_fin == flag) %>% 
  left_join(env_data) %>%
  na.omit() # removes any NAs

if(flag == "HKG"){
  model_data_flag <- model_data_flag %>%
    filter(eez_id != 8603) # Since CHN and HKG have the same cells, if we don't filter out CHN there will be duplicates
...
# prop_fishing_hours_cell = the proportion of that flag country's fishing vessels represented in that cell (per length and year groupings; see below)
...
toc() # half a second for AUS

# Initialize tracking
var_imp_i <- data.frame(importance(model))
n_vars <- nrow(var_imp_i)

qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_{flag}_{n_vars}.qs")))
}
...
if(!file.exists(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_{flag}_27.qs")))){
tic()
  model <- ranger(
  formula = full_model_formula,
  data = train,
  num.trees = 500, # change this to 500? 
  importance = "impurity",
  write.forest = TRUE,
  classification = FALSE
)
toc() 
  ...
  model <- qs::qread(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_{flag}_27.qs")))
...
rmse <- sqrt(mean((test$y_logit - pred_props)^2)) # use RMSE for probabilities instead?
...
while (TRUE) {
  ...
  # Select variables above threshold
    ...
    selected_vars <- unique(c("year", "log_looks", selected_vars)) # always retain year and the ais reception data (for ais data biases)
  ...
  file_path <-   glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_{flag}_{n_vars}.qs"))
 ...
 model_i <- ranger(
  formula = y_logit ~.,
  data = train[, c("y_logit", selected_vars)],
  num.trees = 500, 
  importance = "impurity",
  write.forest = TRUE,
  classification = FALSE
  ...
  rmse_i <- sqrt(mean(test$y_logit - pred_props_i)^2)
...
#  flag = "BRB"
  ...
  paste("y_logit ~", paste(best_train_model$forest$independent.variable.names, collapse = " + "))
)
...
qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_2_models/stage_2_rf_model_full_data_{flag}_{best_model_n}.qs")))
}
