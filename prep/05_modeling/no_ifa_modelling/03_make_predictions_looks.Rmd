---
title: "Predict props to cells per flag, year, length, NO IFA"
...
Make predictions of fishing effort, WITHOUT constraining to IFAs. We will however limit predictions to each flag countries' EEZ
...
Now we need to prep our "looks" dataset 
...
looks_hist <- looks_df %>%
  ...
  left_join(., looks_df) %>%
  mutate(looks_fin = ifelse(is.na(looks), looks_gf, looks)) %>% # now gapfill any missing with the average of that cell across all years
...
full_data <- full_grid %>%
  ...
  filter(eez_sovereign == flag) %>% # we're only gonna make predictions on cells where they can fish - no sea ice, and their own EEZ for artisanal
...
qs::qsave(all_preds, file.path(rdsi_dir, glue("prep/random_forest/artisanal_skylight_sentinel/predictions_no_ifa/yearly/model_preds_{flag}_{yr}.qs"))) # qs is smaller
...
foreach(flag = model_flags, .packages = c("qs", "dplyr", "tidyverse", "glue", "countrycode")) %dopar% {
  ...
  all_files_flag <- list.files(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/predictions_no_ifa/yearly/"), pattern = flag, full.names = TRUE)
  ...
  qs::qsave(all_data_flag, file.path(rdsi_dir, glue("prep/random_forest/artisanal_skylight_sentinel/predictions_no_ifa/model_preds_1950_2017_{flag}.qs")))
}
...
test_preds <- qs::qread("/home/ubuntu/data_storage/prep/random_forest/artisanal_skylight_sentinel/predictions_no_ifa/model_preds_1950_2017_ZAF.qs") %>%
  filter(year == 2017) %>%
  dplyr::group_by(lon, lat) %>%
