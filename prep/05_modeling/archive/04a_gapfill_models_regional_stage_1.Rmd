---
title: "Prep sentinel 2 vessel data"
...
Here we fit stage 1 larger regional models which will be used for gapfilling missing flag countries in the Skylight data. We fit regional models for these 12 regions: 
...
model_formula_rf <- formula(
  presence ~ 
    ...
    # eez_id +
    # fao_id +
    # ocean +  # Spatial categorical variables; because right now we are training on data that does not contain presence for EEZ, FAO areas, meso areas, or oceans. If we incorporate the sentinel 2 data, maybe this will be fixed? 
...
## Run models and variable selection for every region
 ...
 - Currently takes ~40 minutes with 8 cores and 500 trees. 
...
model_data_loop <- model_data %>% 
  ...
  distinct(lon, lat, region, year, length_category, presence) # need to get distinct for the larger regions

regions_to_run <- unique(model_data_loop$region) # get the regions we want to run; only 8 we can run. We will need to do the missing 4 with a global model? 

missing_regions <- setdiff(unique(hist_fish_data$region), regions_to_run) # LatinAmerica, Caribbean, NorthAmerica, NWAfrica will need a global model 
...
foreach(region_loop = regions_to_run, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger")) %dopar% {
    ...
    # region_loop = "SubAfrica"
  model_data_region <- model_data_loop %>%
    dplyr::select(lon, lat, region, length_category, year, presence) %>%
    filter(region == region_loop)
  
  distinct_cats <- model_data_region %>%
    distinct(year, region, length_category)
  
  full_grid <- tidyr::crossing(env_grid, distinct_cats)
  
  full_data <- full_grid %>%
    left_join(env_data, by = c("lon", "lat", "year")) %>%
    left_join(model_data_region, by = c("lon", "lat", "year", "region", "length_category")) %>%
...
model <- ranger(
  ...
  qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_1_models_regional/pruning/stage_1_rf_train_{region_loop}_{n_vars}.qs")))
...
Now apply variable selection methods 
 ...
 - takes ~4 hours with 8 cores 500 trees
...
foreach(region_loop = regions_to_run, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger", "PRROC")) %dopar% {
  ...
  num_vars <- c()
  model_data_region <- model_data_loop %>%
    dplyr::distinct(lon, lat, region, length_category, year, presence) %>%
    filter(region == region_loop)
  ...
  base_model_path <- glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_1_models_regional/pruning/stage_1_rf_train_{region_loop}_23.qs"))
...
repeat {
    ...
    ## add a formula here? 
        file_path <- glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_1_models_regional/pruning/stage_1_rf_train_{region_loop}_{length(selected_vars)}.qs"))
...
Now write code to select the "best" model per the variable selection from above. We will rerun the model with just those variables on the FULL dataset to leverage all of the data from skylight and save
 ...
 - takes ~2 hours to run with 500 trees 

```{r}

# Grab the flags that were run in the folder
stage_1_path <- file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_1_models_regional/pruning")
stage_1_files <- list.files(stage_1_path, full.names = TRUE)
stage_1_regions <- unique(sub(".*stage_1_rf_train_([^_]+).*", "\\1", stage_1_files))

stage_1_path_done <- file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_1_models_regional/")
...
foreach(region_loop = stage_1_regions, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger", "strex")) %dopar% {
  ...
  # Save model
  qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_1_models_regional/stage_1_rf_model_full_data_{region_loop}_{best_model_n}.qs")))
}
