...
Here we fit stage 2 larger regional models which will be used for gapfilling missing flag countries in the Skylight data. We fit regional models for these 12 regions: 
...
model_data_loop <- model_data %>%
  left_join(rousseau_fix, by = c("flag_fin" = "Country")) %>%
  # mutate(region = ifelse(is.na(region), "High seas", region)) %>% # don't worry about high seas for artisanal
 group_by(lon, lat, length_category, region, year) %>%
  summarise(nv = sum(nv, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(!is.na(region)) %>%
  group_by(length_category, region, year) %>%
...
```{r}
regions_to_run <- unique(model_data_loop$region) # get the regions we want to run

# Set up parallel backend
num_cores <- 8 # Use one less than the total available cores
...
foreach(region_loop = regions_to_run, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger")) %dopar% {
  ...
  dplyr::select(lon, lat, region, length_category, year, prop_vessels) %>%
...
n_vars <- nrow(var_imp_i)
  ...
  qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_2_models_regional/pruning/stage_2_rf_train_{region_loop}_{n_vars}.qs")))
...
if(!file.exists(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_2_models_regional/pruning/stage_2_rf_train_{region_loop}_23.qs")))){
tic()
...
toc() 
  ...
  model <- qs::qread(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_2_models_regional/pruning/stage_2_rf_train_{region_loop}_23.qs")))
...
while (TRUE) {
  ...
  file_path <-   glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_2_models_regional/pruning/stage_2_rf_train_{region_loop}_{n_vars}.qs"))
...
stage_2_path <- file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_2_models_regional/pruning")
...
qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_minderoo/stage_2_models_regional/stage_2_rf_model_full_data_{region_loop}_{best_model_n}.qs")))
}
