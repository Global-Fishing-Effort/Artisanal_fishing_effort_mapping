...
# Summary
Here we read in each model created in step 2 and make predictions of fishing effort from those models. 

We stage 1 restrict predictions to each flag state’s EEZ (Watson & Tidd, 2018; Pauly et al., 2020), which assumes domestic operation and may miss cross-border activity under access agreements or IUU in some regions. For stage 1, we treated any non-zero predicted probability as presence. Stage 2 outputs are the per-look detection rate predictions (obtained by back-transforming the logit predictions), with predictions in all cells globally within our Inshore Fishing Areas layer. We combine the two stages to obtain expected detections in each cell by multiplying the Stage 1 outputs by the per-look detection rate predictions and the number of valid satellite looks in that cell and year. Then we convert expected detections to proportions by normalizing within each country-year-length combination, ensuring the sums equal one. We then multiply the national level artisanal fishing vessel count totals (Rousseau et al., 2019, 2024) by each cell within the country-year-length combination to allocate spatially. Because artisanal models create proxy estimates of vessels via detections and predict the number of vessels in each cell, we converted predicted counts to nominal effort by multiplying by country- and length-specific days at sea and mean engine power estimates from Rousseau et al., (2019, 2024), and then applied their technological-creep adjustment (2.61% per year) for effective effort. 
...
ocean_data_historical <- qs::qread(here("data/int/prediction_historical_data/half_degree/chl_yearly_1950_1980.qs")) %>%
 ...
 #  plot(test_ice)
   ...
   mpa_data <- qs::qread(here("data/model_features/mpa/mpa_data_all.qs")) 
  ...
  env_data_test <- env_data %>%
    filter(eez_id == 8671, 
           ifa == 1)
  
env_data_wsm <- env_data %>%
  filter(eez_id == 8671, year == 1966) %>%
  mutate(year = 1967) # for some reason WSM 1967 is not in the IFA data? We will just carry over values from 1966 for WSM...

env_data <- env_data %>%
  dplyr::filter(!(eez_id == 8671 & year == 1967)) %>%
  rbind(env_data_wsm)
...
Now we need to prep our "looks" dataset, so that it extends back to 1950. 

```{r}
looks_df <- qread(here("data/vessels/vessels_prop_05_deg_overpass_all.qs")) %>%
  mutate(year = as.numeric(year)) %>%
  ungroup() %>%
  distinct(pixel_id, year, looks) %>%
  filter(year >= 2019) # use this because 2009-2018 is only skylight, which is only indo pacific 
  

looks_hist <- looks_df %>%
  filter(year >= 2019) %>%
  group_by(pixel_id) %>%
  summarise(looks_gf = mean(looks, na.rm = TRUE), .groups = "drop") %>%
  tidyr::crossing(tibble::tibble(year = 1950:2024)) %>%
  left_join(., looks_df) %>%
  mutate(looks_fin = ifelse(is.na(looks), looks_gf, looks)) %>% # now gapfill any missing with the average of that cell
  dplyr::select(pixel_id, year, looks_fin)
...
full_data <- full_grid %>%
  ...
  left_join(looks_hist) %>%
    dplyr::select(-pixel_id) %>%
    mutate(looks_hat = ifelse(is.na(looks_fin), median(looks_fin, na.rm = TRUE), looks_fin)) %>% # do the median of looks in this region if still missing
  mutate(log_looks = log(looks_hat))

# test <- full_data %>%
#   distinct(lon, lat, L_hat) %>%
#   rast(., type = "xyz") # ok cool
...
# stage2_model <- qs::qread("/home/ubuntu/data_storage/prep/random_forest/artisanal_skylight_sentinel/stage_2_models/pruning/stage_2_rf_train_USA_25.qs")

set.seed(123)
oos_preds <- full_data %>%
  mutate(y_hat = predict(stage2_model, data = .)$predictions) %>%
  mutate(r_hat_plus = plogis(y_hat)) %>%
  mutate(E_hat = r_hat_plus*looks_hat)


# test <- oos_preds %>%
#   filter(length_category == "12-24m") %>%
#   dplyr::select(lon, lat, E_hat) %>%
#   rast(., type = "xyz") %>%
#   plot()
...
# stage1_model <- qs::qread("/home/ubuntu/data_storage/prep/random_forest/artisanal_skylight_sentinel/stage_1_models/pruning/stage_1_rf_train_USA_25.qs")
...
stage_1_preds_rescale <- stage1_preds %>%
    mutate(pred_presence_rescale = case_when(
        pred_presence > 0 ~ 1, # need to make a decision about the cut off. >=0.5 or >0?
        TRUE ~ 0
    )) %>%
...
all_preds <- oos_preds %>%
  dplyr::select(lat, lon, length_category, year, flag_fin, looks_hat, E_hat) %>% # probably select lat, lon, gear, length, year, flag here?
  left_join(., stage_1_preds_rescale) %>% 
    dplyr::select(lon, lat, flag_fin, length_category, year, fao_id, eez_id, eez_sovereign, eez_country_name, looks_hat, E_hat, pred_presence_rescale) %>% 
  ...
  mutate(pred_prop_final = E_hat*pred_presence_rescale*access) %>%
...
foreach(flag = model_flags, .packages = c("qs", "dplyr", "tidyverse", "glue", "countrycode")) %dopar% {
  ...
  all_data_flag <- lapply(all_files_flag, qread) %>%
    ...
    left_join(hist_fish_data %>%
                dplyr::select(-nv)) %>% 
      mutate(nom_active_fishing_days = nv*days_at_sea*mean_engine_p) %>% 
...
test_preds <- qs::qread("/home/ubuntu/data_storage/prep/random_forest/artisanal_skylight_sentinel/predictions/model_preds_1950_2017_WSM.qs") %>%
  filter(year == 1967) %>%
  dplyr::group_by(lon, lat) %>%
