...
all_preds <- oos_preds %>%
  ...
  mutate(access = ifelse(eez_sovereign == flag, 1, 0)) %>% 
...
test_preds <- qs::qread("/home/ubuntu/data_storage/prep/random_forest/artisanal_skylight_sentinel/predictions/model_preds_1950_2017_IND.qs") %>%
  filter(year == 2017) %>%
  dplyr::group_by(lon, lat) %>%
