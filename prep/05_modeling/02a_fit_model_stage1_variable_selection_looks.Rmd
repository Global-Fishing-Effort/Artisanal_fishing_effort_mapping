...
We use the data compiled in the previous scripts to fit a random forest classification model to predict the presence (1) or absence (0) of fishing vessels in every cell. We will do this for each flag country individually.  
...
test <- ocean_data %>%
  filter(year == 2015) %>%
  dplyr::select(pixel_id, sst_c_mean) %>%
...
Run full models first 
 -  With 100 trees it only takes ~4 mins
...
foreach(flag = flags_to_run, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger")) %dopar% {
  ...
  model_data_flag <- model_data %>%
    dplyr::select(pixel_id, lon, lat, flag_fin, length_category, year, looks, log_looks, presence) %>%
    filter(flag_fin == flag) # filter for specific flag from vessel count data
  
  # test <- model_data_flag %>%
  #   distinct(lon, lat, log_looks) %>%
  #   rast(.,type="xyz") # ok cool, looks good for USA. 
...
Now apply variable selection methods 
 - Takes ~6 mins with 20 cores 100 trees
...
foreach(flag = flags, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger", "PRROC")) %dopar% {
   ...
   # flag = "BRB"
    ...
    num_vars <- c()
  model_data_flag <- model_data %>%
    dplyr::select(lon, lat, flag_fin, length_category, year, looks, log_looks, presence) %>%
  ...
  if(!file.exists((glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_1_models/pruning/stage_1_rf_train_{flag}_27.qs"))))){
  ...
  case.weights = train$looks
    ...
    model <- qs::qread(glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_1_models/pruning/stage_1_rf_train_{flag}_27.qs")))
}
 ...
 while (TRUE) {
   ...
   selected_vars <- var_imp_i %>% 
    ...
    selected_vars <- unique(c("year", "log_looks", selected_vars)) # always retain year and the looks data
...
expanded_formula <- as.formula(
  ...
  tic()
    model <- ranger(
        ...
        probability = TRUE,
        write.forest = TRUE,
        case.weights = data_random_forest$looks 
      )
