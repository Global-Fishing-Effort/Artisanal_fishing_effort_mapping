...
foreach(flag = flags_to_run, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger")) %dopar% {
 ...
 # model <- qs::qread(glue(file.path(rdsi_dir, "prep/random_forest/stage_1_models/pruning/stage_1_rf_train_{flag}_27.qs")))
  ...
  qs::qsave(model, glue(file.path(rdsi_dir, "prep/random_forest/artisanal_skylight_sentinel/stage_1_models/pruning/stage_1_rf_train_{flag}_{n_vars}.qs")))
...
## we should move CHN to the front of the list of flags. These two take the longest to model, so should be in the first batch of cores. 
