...
qs::qsave(panel_save, here("data/model_features/mpa/mpa_data.qs"))
...
mpa_data <- data_grid %>%
  ...
  left_join(qs::qread(here("data/model_features/mpa/mpa_data.qs")) %>% dplyr::select(pixel_id, year, mpa)) %>%
...
# 
# n_cores <- min(detectCores() - 1, length(years))
# 
# nearest_years_list <- mclapply(
#   years,
#   function(yr) {
#     # MPAs implemented by or before this year
#     mpa_year <- mpa_est %>%
#       filter(!is.na(est_year), est_year <= yr)
# 
#     if (nrow(mpa_year) == 0) {
#       # no MPAs yet → NA
#       return(tibble(
...
#     }
# 
#     mpa_sf <- st_as_sf(mpa_year, coords = c("lon", "lat"), crs = 4326)
# 
#     # nearest neighbor index for each grid cell
#     nn <- st_nn(grid_sf, mpa_sf, k = 1, returnDist = FALSE)
# 
#     nn_idx <- vapply(
#       nn,
#       function(v) if (length(v) == 0) NA_integer_ else v[1],
#       integer(1)
#     )
# 
#     nearest_est_year <- mpa_year$est_year[nn_idx]
# 
#     years_since <- ifelse(
#       is.na(nearest_est_year),
#       NA_integer_,
#       pmax(0L, yr - nearest_est_year)
#     )
# 
#     tibble(
#       pixel_id = data_grid$pixel_id,
...
# )
# 
# nearest_years_timevarying <- bind_rows(nearest_years_list)
# 
```


```{r}
# 0.5° template derived from your panel
lon_vals <- sort(unique(mpa_data$lon))
lat_vals <- sort(unique(mpa_data$lat))

res_x <- median(diff(lon_vals))  # should be 0.5
res_y <- median(diff(lat_vals))  # should be 0.5
...
qs::qsave(mpa_data_filled, here("data/model_features/mpa/mpa_data_all.qs"))
