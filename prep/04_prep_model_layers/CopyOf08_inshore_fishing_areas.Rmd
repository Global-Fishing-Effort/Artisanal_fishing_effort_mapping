...
```{r}
# 1) Sea mask from gfw_vars in lon/lat and project to a metric CRS for distance
sea_ll  <- gfw_vars %>% transmute(lon, lat, sea = as.integer(elevation_m < 0)) %>% rast(type = "xyz", crs = mollweide_projection)
sea_ll[sea_ll == 0] <- NA                             # land -> NA, sea -> 1

crs_proj <- "ESRI:54030"
sea_proj <- project(sea_ll, crs_proj, method = "near")  # 1 at sea, NA on land (metric template)

# 2) Make a clean *source* from populated coastal LAND: 1 on sources, NA elsewhere
#    (distance() measures to non-NA cells)
inh_src <- ifel(inhabited_coast, 1, NA) %>%
  project(., crs_proj)

# 3) Distance from sea to populated coastal land, per year (meters), keep sea only, then ≤50 km
dist_to_popcoast_m <- distance(project(inh_src, crs_proj, method = "near")) 

ext(dist)
dist_to_popcoast_m <- mask(dist_to_popcoast_m, sea_proj)      # sea only

within50_proj <- dist_to_popcoast_m <= 50000                  # ≤ 50 km from populated coastal LAND (sea only)

# 4) (Optional) add the SAUP depth OR rule from gfw_vars
#    Base IFA from your static vars: within 50 km of shore OR depth ≥ −200 m (sea only)
gfw_base_df <- gfw_vars %>%
  transmute(lon, lat,
            ifa_basic_sea = (distance_from_shore_m  <= 50) | (elevation_m >= -200),
            sea = elevation_m < 0)
gfw_base_df$ifa_basic_sea <- as.integer(gfw_base_df$ifa_basic_sea & gfw_base_df$sea)
ifa_basic_ll  <- rast(gfw_base_df %>% select(lon, lat, ifa_basic_sea), type = "xyz")
ifa_basic_proj <- project(ifa_basic_ll, crs_proj, method = "near")

# final inhabited IFA = (≤50 km from populated coastal LAND) OR (ifa_basic), sea only
ifa_inhab_proj <- (within50_proj | ifa_basic_proj) & sea_proj

# 5) Project back to lon/lat, extract to your grid, tidy to long
ifa_inhab_ll <- project(ifa_inhab_proj, "EPSG:4326", method = "near")

pts  <- vect(data_grid[, c("lon","lat")], geom = c("lon","lat"), crs = "EPSG:4326")
vals <- terra::extract(ifa_inhab_ll, pts)

ifa_final <- vals %>%
  mutate(pixel_id = data_grid$pixel_id) %>%
  pivot_longer(-c(ID, pixel_id), names_to = "year", values_to = "ifa") %>%
  mutate(year = as.integer(gsub("^.*?(\\d{4}).*$", "\\1", year)),
         ifa  = as.integer(replace_na(ifa, FALSE))) %>%
  select(pixel_id, year, ifa) %>%
  arrange(pixel_id, year)

# Quick check map (should be a ~50 km sea band off *populated coastal land*)
ifa_final %>%
  filter(year == 2017) %>%
  left_join(select(data_grid, pixel_id, lon, lat), by = "pixel_id") %>%
  select(lon, lat, ifa) %>%
  rast(type = "xyz") %>%
  plot(main = "Sea cells ≤50 km from populated coastal LAND (OR depth rule)")
```
