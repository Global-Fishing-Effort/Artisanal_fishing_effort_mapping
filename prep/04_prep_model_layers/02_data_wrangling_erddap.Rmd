...
Spatially and temporally aggregate errdap data to 0.5 grid
...
library(strex)
library(FNN)
source(here::here("prep/04_prep_spatial_layers_model/_functions_data_wrangling_erddap.R"))
source(here("R/dir.R"))
...
## save 
# qs::qsave(sst_data_imputed, here("data/model_features/sst_data.qs")) # only 35 MB - cool 

write.csv(sst_data_imputed, here("data/model_features/sst_data.csv"), row.names = FALSE) # only 80 mb 

## might need to consider filling in the pixel ids with no information.. right now we are just filling in the cells we could grab from errdap. E.g. there are some pixel ids like 1 that don't exist in the raw data. So we would need to cross those pixel ids with each year and then fill in with the nearest neighbor pixel.
```
...
# ## save 
qs::qsave(chl_data_imputed, here("data/model_features/chl_data.qs")) # only 35 MB - cool 

# write.csv(chl_data_imputed, here("data/model_features/chl_data.csv"), row.names = FALSE) # too big for github; >100 mb

```


# Code to prep the same data to a 1 degree by 1 degree grid if needed
