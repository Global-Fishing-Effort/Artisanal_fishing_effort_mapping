...
vessels_2025_july <- fread(file.path(rdsi_raw_dir, "global_fishing_watch/sentinel_2_data/sentinel2_vessel_detections_pipev3_20250705.csv"))
...
vessels_2025_july <- vessels_2025_july %>%
    mutate(month = as.character(as_date(floor_date(as_date(detect_timestamp), unit = "month"))))
...
vessels_july_df <- vessels_2025_july %>%
  dplyr::select(lon, lat, length_class, presence_score, nonvessel_score, likely_infrastructure, potential_ice, matching_score, matching_confidence, mmsi, month) %>%
...
vessels_df_fin <- rbind(non_mmsi_keep, mmsi_keep) %>%
  dplyr::select(lon, lat, length_class, h3_id, month)


```

We will need a file for each year that is lon, lat, length_class, and year. Each row represents a vessel detection. 
